// from https://github.com/nopnop2002/esp-idf-ili9340
#include <string.h>
#include <math.h>
...
#if 0
	static uint8_t Byte[1024];
	assert(size <= sizeof(Byte)/2);
#if 0 // original version with swap
	int index = 0;
	for(int i=0;i<size;i++) {
...
#else // no swap
        memcpy(&Byte, colors, size*2);
#endif
	gpio_set_level( dev->_dc, SPI_Data_Mode );
        return spi_master_write_byte( dev->_TFT_Handle, (uint8_t*) &Byte, size*2);

#else
#if 0
        // copy to buffer
	static uint16_t Word[1024/2];
	assert(size <= sizeof(Word));
	memcpy(&Word, colors, size*2);
	gpio_set_level( dev->_dc, SPI_Data_Mode );
	return spi_master_write_byte( dev->_TFT_Handle, (uint8_t*) &Word, size*2);
#else // no copy
        gpio_set_level( dev->_dc, SPI_Data_Mode );
        return spi_master_write_byte( dev->_TFT_Handle, (uint8_t*) colors, size*2);
#endif
#endif
}
...
void delayMS(int ms) {
	...
	ESP_LOGD(TAG, "ms=%d _ms=%d portTICK_PERIOD_MS=%lu xTicksToDelay=%lu",ms,_ms,portTICK_PERIOD_MS,xTicksToDelay);
...
void lcdInit(TFT_t * dev, uint16_t model, int width, int height, int offsetx, int offsety)
{
	...
	if (dev->_model == 0x9340 || dev->_model == 0x9341 || dev->_model == 0x7735) {
		...
		spi_master_write_data_byte(dev, 0xC5);
                ...
                // added
		spi_master_write_comm_byte(dev, 0xC2);	//Power Control 3
		spi_master_write_data_byte(dev, 0x0A);
		spi_master_write_data_byte(dev, 0x00);
		spi_master_write_comm_byte(dev, 0xC3);	//Power Control 4
		spi_master_write_data_byte(dev, 0x8A);
		spi_master_write_data_byte(dev, 0x2A);
		spi_master_write_comm_byte(dev, 0xC4);	//Power Control 5
		spi_master_write_data_byte(dev, 0x8A);
		spi_master_write_data_byte(dev, 0xEE);
		spi_master_write_comm_byte(dev, 0xC5);	//VMCTR1
...
// BGR mode only requires swap for BMP LE RGB565 data
// seems like specification is wrong,  BGR <--> RGB, big endian format
		spi_master_write_data_byte(dev, 0xA8);	// landscape Right top start, BGR color filter panel
		//spi_master_write_data_byte(dev, 0x00);//Right top start, RGB color filter panel
// RGB mode ends up with swapped R/B
//		spi_master_write_data_byte(dev, 0xA0);	// landscape Right top start, RGB color filter panel

		spi_master_write_comm_byte(dev, 0x3A);	//Pixel Format Set
		spi_master_write_data_byte(dev, 0x55);	//65K color: 16-bit/pixel
//		spi_master_write_data_byte(dev, 0x05);	//65K color: 16-bit/pixel
